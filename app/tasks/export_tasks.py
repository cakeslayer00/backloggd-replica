...
def generate_weekly_stats(self):
    print("[STATS] Generating weekly statistics")
    ...
    try:
        ...
        stats = {
            ...
            "total_backlog_entries": session.query(
                func.count(BacklogEntry.id)
            ).scalar(),
            "total_reviews": session.query(func.count(Review.id)).scalar(),
            "new_users_this_week": session.query(func.count(User.id))
            .filter(User.created_at >= week_ago)
            .scalar(),
            "new_entries_this_week": session.query(func.count(BacklogEntry.id))
            .filter(BacklogEntry.created_at >= week_ago)
            .scalar(),
            "new_reviews_this_week": session.query(func.count(Review.id))
            .filter(Review.created_at >= week_ago)
            .scalar(),
            "games_completed": session.query(func.count(BacklogEntry.id))
            .filter(BacklogEntry.status == "completed")
            .scalar(),
        }
